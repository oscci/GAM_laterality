...
Models vary in 2 key respects:
a) which time terms included in slope estimates: sec, relativetime, epoch
b) whether gamma1 or POI used to estimate LI (from interaction with side)

Various models to be compared in terms of :
1. Precision of LI estimate, as estimated from the SE
2. Agreement with Doppler LI (nb this is not gold standard though)
3. Agreement with fMRI LI (though note small N for this)
...
Alternatives include additions to this model by inclusion of relative time and epoch terms.

Previous script epoched_plot.Rmd based on Paul's epoched_plot.R has become unwieldy and inefficient, so here aim to streamline it.
...
origWG$fmri_wg_whole<-NA
origWG$fmriID[w]<-myf$ID

origWG$fmri_wg_MCA[w]<-myf$fMRI_new_wg_MCA
origWG$fmri_wg_whole[w]<-myf$fMRI_new_wg_wholecor.test(origWG$fMRI_wg_MCA,origWG$LI,use="complete.obs")


plot(origWG$LI,origWG$fMRI_wg_whole,col=as.factor(origWG$lat)) #Doppler LI vs fmri whole hem
```
...
```{r modelfit.save}
  modelfit<- function(longdata,summary.data,gam.method){
    ...
    if(gam.method==1){
      ...
      myfit <- glm(y~gamma1+gamma2+sec+I(sec^2)+I(sec^3)+side+gamma1*side,data=longdata)
    }
    ...
    if(gam.method==12){
      longdata$epoch<-as.factor(longdata$epoch) #instead of time, use relativetime and epoch (latter as factor).
      myfit <- gam(y~s(sec)+s(sec,by=side)+gamma1+gamma2+side+gamma1*side+gamma2*side,data=longdata)
    }
...
summary.data<-cbind(summary.data,addbit)
  ...
  gamlist<-c('Canonical hrf 5s from marker1',
             ...
             'with relative timexepoch and POI',
             'with sec,relative timexepoch and gamma',
             'with sec,relative timexepoch and POI')
  
  gammethods<- c(1,3,4,7,8,9,10,11,12) #can have a vector here if need be
...
```{r doplots}
      ...
      g3<-ggplot(longdata,aes(y=y,x=sec,colour=side))+
        ...
        annotate(geom="text", x=550, y=85, label=paste0(gamlist[gam.method],": baselined=",bindex[usebdata]))+
      ...
      if(usebdata==2){
        myfilepath<-here(paste0('method',gam.method,'_GAM_figs_unbaselined'))
      }
    ...
    filename<-paste0('method',gam.method,bindex[usebdata],'_summary.csv')
    write.csv(summary.data,filename,row.names=F)
  }         
  
  #Some preliminary explorations
  #Param 4 is parameter for interaction
  myr<-round(cor(summary.data$LI,summary.data$LI.GAM.est),3)
  plot(summary.data$LI,summary.data$LI.GAM.est,col=as.factor(summary.data$lat),main=myr)
  ...
  plot(summary.data$LI.GAM.est,log(summary.data$p_interaction),col=as.factor(summary.data$lat))
  r=cor.test(summary.data$LI,summary.data$LI.GAM.est)$estimate
```
...
```{r withfmritry}
  ...
  myf<-read.csv('LI_fmri.csv')
  
  mymodel<-'3n'
  myname<-paste0('method',mymodel,'_summary.csv')
  mym3<-read.csv(myname)
  w<-which(mym3$Filename %in% myf$ID)
  myf$LIest3<-mym3$LI.GAM.est[w]
  myf$lat<-mym3$lat[w]
  plot(myf$LIest3,myf$LI,col=as.factor(myf$lat))
  cor.test(myf$LIest3,myf$LI)
  
  myf$LIest3 <- -myf$LIest3
  plot(myf$fTCD..GAM.method,myf$LIest3,col=as.factor(myf$lat))
  
  psych::pairs.panels(myf[,c('LIest3','LI','fMRI..temporal.ROI','fMRI..frontal.ROI','fMRI..MCA.ROA')])
  
```
  
  
```{r comparemethods}
  m3<-read.csv("method3n_summary.csv")
  m4<-read.csv("method4n_summary.csv")
  m8<-read.csv("method8n_summary.csv")
  m9<-read.csv("method9n_summary.csv")
  m10<-read.csv("method10n_summary.csv")
  
  compdata<-as.data.frame(cbind(m3$Filename,m3$lat,m3$LI,m3$se,-m3$LI.GAM.est,m3$LIest.se,m3$R2,m3$p_interaction,-m4$LI.GAM.est,m4$LIest.se,m4$R2,m4$p_interaction,-m8$LI.GAM.est,m8$LIest.se,m8$R2,m8$p_interaction,-m9$LI.GAM.est,m9$LIest.se,m9$R2,m9$p_interaction,-m10$LI.GAM.est,m10$LIest.se,m10$R2,m10$p_interaction))
  colnames(compdata)<-c('ID','lat','LI','LIse','m3LI','m3se','m3R2','m3_p','m4LI','m4se','m4R2','m4_p','m8LI','m8se','m8R2','m8_p','m9LI','m9se','m9R2','m9_p','m10LI','m10se','m10R2','m10_p')
  mycols<-c('LI','LIse','m3LI','m3se','m3R2','m3_p','m4LI','m4se','m4R2','m4_p','m8LI','m8se','m8R2','m8_p','m9LI','m9se','m9R2','m9_p','m10LI','m10se','m10R2','m10_p')
  for (mycol in 3:24){
    compdata[,mycol]<-as.numeric(compdata[,mycol])
  }
  
  plot(compdata$LI,compdata$m3LI,col=as.factor(compdata$lat),main='Model 3: PT orig')
  text(-2,3.5,paste0('mean Dop LI = ',round(mean(compdata$LI),3)),adj=0)
  text(-2,3.1,paste0('m3 LI = ',round(mean(compdata$m3LI),3)),adj=0)
  text(-2,2.7,paste0('mean Dop se = ',round(mean(compdata$LIse),3)),adj=0)
  text(-2,2.3,paste0('m3 se = ',round(mean(compdata$m3se),3)),adj=0)
  
  text(0,-1.5,paste0('mean m3R2 = ',round(mean(compdata$m3R2),3)),adj=0)
  text(0,0,paste0('r = ',round(cor(compdata$LI,compdata$m3LI),3)),adj=0)
  
  plot(compdata$LI,compdata$m4LI,col=as.factor(compdata$lat),main='Model 4: PT with epoch')
  text(-2,3.5,paste0('mean Dop LI = ',round(mean(compdata$LI),3)),adj=0)
  text(-2,3.1,paste0('m4 LI = ',round(mean(compdata$m4LI),3)),adj=0)
  text(-2,2.7,paste0('mean Dop se = ',round(mean(compdata$LIse),3)),adj=0)
  text(-2,2.3,paste0('m4 se = ',round(mean(compdata$m4se),3)),adj=0)
  text(0,-2,paste0('r = ',round(cor(compdata$LI,compdata$m4LI),3)),adj=0)
  text(0,-1.5,paste0('mean m4R2 = ',round(mean(compdata$m4R2),3)),adj=0)
  
  plot(compdata$LI,compdata$m8LI,col=as.factor(compdata$lat),main='Model 8: POI with epoch')
  text(-2,3.5,paste0('mean Dop LI = ',round(mean(compdata$LI),3)),adj=0)
  text(-2,3.1,paste0('m8 LI = ',round(mean(compdata$m8LI),3)),adj=0)
  text(-2,2.7,paste0('mean Dop se = ',round(mean(compdata$LIse),3)),adj=0)
  text(-2,2.3,paste0('m8 se = ',round(mean(compdata$m8se),3)),adj=0)
  text(0,0,paste0('r = ',round(cor(compdata$LI,compdata$m8LI),3)),adj=0)
  text(0,-1.5,paste0('mean m8R2 = ',round(mean(compdata$m8R2),3)),adj=0)
  
  plot(compdata$LI,compdata$m9LI,col=as.factor(compdata$lat),main='Model 9: gamma with sec epoch')
  text(-2,3.5,paste0('mean Dop LI = ',round(mean(compdata$LI),3)),adj=0)
  text(-2,3.1,paste0('m9 LI = ',round(mean(compdata$m9LI),3)),adj=0)
  text(-2,2.7,paste0('mean Dop se = ',round(mean(compdata$LIse),3)),adj=0)
  text(-2,2.3,paste0('m9 se = ',round(mean(compdata$m9se),3)),adj=0)
  
  text(0,-1.5,paste0('mean m9R2 = ',round(mean(compdata$m9R2),3)),adj=0)
  text(0,-1.0,paste0('r = ',round(cor(compdata$LI,compdata$m9LI),3)),adj=0)
  
  plot(compdata$LI,compdata$m10LI,col=as.factor(compdata$lat),main='Model 10: POI with epoch')
  text(-2,3.5,paste0('mean Dop LI = ',round(mean(compdata$LI),3)),adj=0)
  text(-2,3.1,paste0('m10 LI = ',round(mean(compdata$m10LI),3)),adj=0)
  text(-2,2.7,paste0('mean Dop se = ',round(mean(compdata$LIse),3)),adj=0)
  text(-2,2.3,paste0('m10 se = ',round(mean(compdata$m10se),3)),adj=0)
  text(0,0,paste0('r = ',round(cor(compdata$LI,compdata$m10LI),3)),adj=0)
  text(0,-1.5,paste0('mean m10R2 = ',round(mean(compdata$m10R2),3)),adj=0)
