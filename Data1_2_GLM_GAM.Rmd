...
eg <- 2 # specify which example (1 is Bruckert Dataset, and 2 is Woodhead repeated measures)
if(eg==1){
...
#here is automatically set to project directory
  ...
  #NB the last 7 participants were initially excluded but it is not clear why, so they have been appended to the origdata file, together with handedness/sex data
 ntrials<-15
...
origdata<-origdata[,1:3] #script was written with hard coding of columns for just 3 cols, so remove the demographics for now
...
modelcompare[,1]<- c('Mean LI','Within-subject SE LI','R2','AIC','BIC','% Right','% Bilateral','% Left','r with POI Average','r with fMRI LI (N = 31)','r with fMRI L-R')
...
LIdata1<-summary.data[1:nrow(origdata),LIcols]
LIdata2<-summary.data[(nrow(origdata)+1):(nrow(origdata)*2),LIcols]

latdata1<-summary.data[1:nrow(origdata),latcols]+2
latdata2<-summary.data[(nrow(origdata)+1):(nrow(origdata)*2),latcols]+2
...
for (i in 1:4){
 consistent<-rep(0,nrow(origdata))
...
write.csv(modelcompare,'modelcompare.csv.',row.names=F)
}
```
