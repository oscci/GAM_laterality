...
paul_gam<-function(rawdata,stim1_length_samples,stim2_length_samples,peaklist,summary.data,usebdata,order){
  ...
  #add column indicating which task
  longdata$task<-0
  task1<-which(task_order==1)
  longdata$task[which(longdata$epoch %in% task1)]<-1
    task2<-which(task_order==2)
  longdata$task[which(longdata$epoch %in% task2)]<-2
    task3<-which(task_order==3)
  longdata$task[which(longdata$epoch %in% task3)]<-3
  longdata$taskf<-as.factor(longdata$task)
  levels(longdata$taskf)<-c('S','L','W')
...
```{r modelfit.save}
  modelfit<- function(longdata,summary.data,gam.method){
    ...
    if(gam.method==6){
      ...
      myfit <- gam(y~s(sec)+s(relativetime)+s(relativetime,by=epoch)+gamma1+gamma2+side+gamma*side*taskf,data=longdata)
    }
