...
ftcd_preprocess<-function(path1,path2,filename1,filename2,j,summary.data){ #this just runs one person at a time 
  ...
  saveepochedavg<-1 #keep this at zero unless you want to save the averaged baselined files (the ones used for ftcd LI computation)
  ...
  includeepochs<-summary.data[j,2:61] #0 or 1 for each trial - 0 means exclude for signal dropout or failure to do task
  excludeepochs<-which(includeepochs==0)
...
```{r modelfit.save}
  modelfit<- function(longdata,summary.data,gam.method){
    ...
    if(gam.method==6){
      ...
      myfit <- gam(y~s(sec)+s(relativetime)+s(relativetime,by=epoch)+gamma1+gamma2+gamma1*side*taskf,data=longdata)
...
summary.data$Npts<-NA #initialise column to show N points analysed
summary.data$Filename<-sprintf('%0.3d', summary.data$Filename)

 addbit<-data.frame(matrix(NA,nrow=nrow(summary.data),ncol=7))
nunames<-c('LI_interactS','se_interactS','LI_interactW','se_interactW','R2','AIC','BIC')
...
summary.data<-cbind(summary.data,addbit)
  ...
  gammethods<- c(6) #can have a vector here if need be. We designate model with interaction with task as model 6
  ...
  for (gam.method in gammethods){
   ...
   endtime<-Sys.time()
    #   
      ...
      sp<-s$p.coeff[12:13]
      se<-s$se[12:13]
      summary.data$LI_interactS<-sp[1]
      summary.data$se_interactS<-se[1]
      summary.data$LI_interactW<-sp[2]
      summary.data$se_interactW<-se[2]
      summary.data$R2<-s$r.sq
      summary.data$AIC<-AIC(myfit)
      summary.data$BIC<-BIC(myfit)
    ...
    write.csv(summary.data,'summary.data.ex3.csv',row.names=F)
