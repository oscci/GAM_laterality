...
gammethods<- 3 #can have a vector here if need be
#method 2 is boxcar with POI
usebdata=2 #if 2, then use unbaselined heartbeatcorrected data, otherwise baselined by epoch
...
```{r withfmritry}
#Checking why correl with new method 3 so much worse than with old method 3 from PT originalscript
#First idea is that it might just be because different subset of people included - only 27!

myf<-read.csv('LI_fmri.csv')


mym3<-read.csv('method3_summary.csv')
myf$LIest3<-mym3$side.interact[w]
myf$lat<-mym3$lat[w]
plot(myf$LIest3,myf$LI)
cor.test(myf$LIest3,myf$LI)


plot(myf$fTCD..GAM.method,myf$LIest3,col=as.factor(myf$lat))
#It's clear that the old and new methods are not the same - though most fall on a straight line, but some depart massively. Suggests something about those cases is askew.
#So next task is to look at specific cases with big mismatch.
