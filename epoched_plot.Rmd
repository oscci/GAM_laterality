...
doGAM <-function(longdata,hrf,summary.data,usebdata){
  ...
  summary.data$LIest.se[j] <- summary(myfit)$se[length(summary(myfit)$p.coeff)] #here p.coeff are the parameter coefficients, and we put the SE in for the last one, which is the interaction
...
paul_gam<-function(rawdata,stim1_length_samples,stim2_length_samples,peaklist,summary.data,usebdata,order,gam.method){
  ...
  summary.data$method[j] <- paste0(gam.method,bindex[usebdata])
  w<-which(colnames(summary.data)=='param1')
  ncoeffs<-length(sp)
  summary.data[j,w:(w+ncoeffs-1)] <- anova(myfit)$'p.coeff' 
  colnames(summary.data)[(w+ncoeffs-1)]<-'LI.GAM.est'
  summary.data$LIest.se[j]<-s$se[ncoeffs]
...
```{r run-analysis}
  ...
  #We make enough cols for 5 parameters, though there may only be 4 depending on model
  #The last parameter will be renamed as LI.GAM.est. We include a col for its SE as well
  summary.data<-data.frame(matrix(NA,nrow=nrow(origWG),ncol=15))
  names(summary.data)<-c('ID',paste0('param',1:6),'LIest.se','method','AIC_glm','AIC_gam','BIC_glm','BIC_gam','p_interaction','R2')
...
gammethods<- c(12) #can have a vector here if need be
