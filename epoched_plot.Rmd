...
pinteract<-anova(myfit2b,myfit2,test="Chisq") #check if sig interaction
...
```{r run-analysis}
gamlist<-c('Canonical hrf 5s from marker1','Boxcar POI','PT original gamma')

gam.method <- 3 #method 1 uses canonical hrf for 5 s duration from marker onset
...
if(gam.method==3){ #using the more complex GAM
...
myr<-round(cor(summary.data$LI,summary.data$param4),3)
plot(summary.data$LI,summary.data$param4,col=as.factor(summary.data$lat),main=myr)
...
plot(summary.data$param4,log(summary.data$p_interaction),col=as.factor(summary.data$lat))
...
# longdata<-longdata[-w,]
# numod <- gam(y ~ s(epoch)+s(relativetime)+side+s(POI)+s(side*POI), k=5, bs="cr", data=longdata, method="REML", family="gaussian")
# 
# 
