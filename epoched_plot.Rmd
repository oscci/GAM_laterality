...
#fit single GAM for each individual.
    ...
    plot(myepoched_average$time,myepoched_average$Lmean,type='l',col='blue')
    lines(myepoched_average$time,myepoched_average$Rmean,type='l',col='red')
...
```{r convolve}
    ...
    plot(rawdata$sec,gamma1,type='l',col='blue')
   lines(rawdata$sec,gamma2,type='l',col='red')
 ...
 mygdata<-data.frame(y=c(rawdata$heartbeatcorrected_L,rawdata$heartbeatcorrected_R),stim1=c(gamma1,gamma1),stim2=c(gamma2,gamma2),t=c(rawdata$sec,rawdata$sec),signal=rep(c(1,-1),each=length(gamma1)))
 
colnames(mygdata)<-c('y','gamma1','gamma2','sec','side')
...
levels(mygdata$side)<-c("Left","Right")
    ...
    mygdatax<-mygdata[c(peaklist,peaklist+(dim(mydata)[1]/2+1)),] #db renamed to mydatax to preserve original mydata
    
    plot(mygdatax$sec[mygdatax$side=='Left'],mygdatax$y[mygdatax$side=='Left'],type='l',col='blue')
lines(mygdatax$sec[mygdatax$side=='Right'],mygdatax$y[mygdatax$side=='Right'],type='l',col='red')
    ...
    # fit gam model 
    myfit <- gam(y~s(sec)+s(sec,by=side)+gamma1+gamma2+side+gamma1*side,data=mygdatax)
    #print("2")
    myfit2 <- glm(y~gamma1+gamma2+sec+I(sec^2)+I(sec^3)+side+gamma1*side,data=mygdatax)
    #print("3")
    #check fit of the model
    #print(appraise(myfit))
    
    #Try without gamma2
    myfit1a <- gam(y~s(sec)+s(sec,by=side)+gamma1+side+gamma1*side,data=mygdatax)
    #print("2")
    myfit2a <- glm(y~gamma1+sec+I(sec^2)+I(sec^3)+side+gamma1*side,data=mygdatax)
    ...
    myplotdat<-data.frame(y=mygdatax$y,x=mygdatax$sec,fitted=predict(myfit),Side=mygdatax$side)#$gam, type = "response"
    
    #plot the time series for each individual per side.
    g3<-ggplot(myplotdat,aes(y=y,x=x,colour=Side))+geom_point(aes(colour=Side),cex=.5,alpha=0.2)+geom_line(aes(y=fitted))+theme_bw()+theme(text=element_text(size=14))+ ylab('Normalised CBFV (cm/s)') + xlab('time(s)')
