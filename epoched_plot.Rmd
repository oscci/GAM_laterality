...
paul_gam<-function(rawdata,stim1_length_samples,stim2_length_samples,peaklist,summary.data,usebdata,order,gam.method){
    ...
    my_des<-cbind(my_des,rep(1:0,each=length(gamma1))) #is this row wrongly duplicated?
    ...
    longdata<-mydata[c(peaklist,peaklist+(dim(mydata)[1]/2+1)),] #mydata is already long form by this point
    ...
    # fit gam model 
    myfit <- gam(y~s(t)+s(t,by=signal)+stim1+stim2+signal+stim1*signal,data=longdata)
    #print("2")
    myfit2 <- glm(y~stim1+stim2+t+I(t^2)+I(t^3)+signal+stim1*signal,data=longdata)
    #print("3")
    #check fit of the model
    #print(appraise(myfit))
    
    #Try without interaction with side
myfit2b <- glm(y~stim1+stim2+t+I(t^2)+I(t^3)+signal,data=longdata)
...
if(gam.method!=3){
hrf<- fmri.stimulus(scans = dim(rawdata)[1],
                    onsets = which(diff(rawdata$stim1_on)==1), 
                    durations = 125, 
                    TR = 1,
                    scale=1,
                    type='canonical')
...
mydata<-rawdata[c(peaklist,peaklist+(dim(rawdata)[1]/2+1)),]
...
if(gam.method==3){
  gamreturn<-paul_gam(rawdata,stim1_length_samples,stim2_length_samples,peaklist,summary.data,usebdata,order,gam.method)
