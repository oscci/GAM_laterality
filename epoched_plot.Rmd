...
myfit2b <- glm(y~hrf+sec+I(sec^2)+I(sec^3)+side,data=longdata)
...
paul_gam<-function(rawdata,stim1_length_samples,stim2_length_samples,peaklist,summary.data,usebdata,order,gam.method){
  ...
  #  mydata<-data.frame(y=c(rawdata$heartbeatcorrected_L,rawdata$heartbeatcorrected_R),stim1=my_des[,1],stim2=my_des[,2],t=my_des[,4],side=as.factor(my_des[,7]),stim1_side=my_des[,8])
    ...
    #print("2")
    longdata$epoch<-as.factor(longdata$epoch)
  ##  myfit <- gam(y~s(relativetime)+s(relativetime,by=epoch)+gamma1+gamma2+side+gamma1*side,data=longdata)
  ##     myfit1b <- gam(y~s(relativetime)+s(relativetime,by=epoch)+gamma1+gamma2+side,data=longdata)

 #  myfit <- gam(y~s(relativetime)+s(relativetime)+gamma1+gamma2+side+gamma1*side,data=longdata)
 # myfit1b <- gam(y~s(relativetime)+s(relativetime)+gamma1+gamma2+side,data=longdata)
 
 #model type 5
   # myfit <- gam(y~s(relativetime)+s(relativetime)+POI+side+POI*side,data=longdata)
   # myfit1b <- gam(y~s(relativetime)+s(relativetime)+POI+side,data=longdata)
   
    #model type 6
   myfit <- gam(y~s(sec)+s(relativetime)+POI+side+POI*side,data=longdata)
   myfit1b <- gam(y~s(sec)+s(relativetime)+POI+side,data=longdata)
    ...
    #Try without interaction with side
myfit2b <- glm(y~gamma1+gamma2+sec+I(sec^2)+I(sec^3)+side,data=longdata)

pinteract<-anova(myfit,myfit1b,test="Chisq") #check if sig interaction
    
    summary.data$method[j] <- gam.method
   w<-which(colnames(summary.data)=='param1')
ncoeffs<-length(anova(myfit)$'p.coeff')
   summary.data[j,w:(w+ncoeffs-1)] <- anova(myfit)$'p.coeff'  #myfit$lme$coefficients$fixed[1:6]
#for model with just one bold term, these correspond to intercept, hrf,side, hrf*side and 5th is blank
...
```{r run-analysis}
gamlist<-c('Canonical hrf 5s from marker1','Boxcar POI','PT original gamma','PT modified gamma','PT with POI')

gam.method <- 5 #method 1 uses canonical hrf for 5 s duration from marker onset
...
write.csv(summary.data,'summary.csv',row.names=F)

#Some preliminary explorations
#Param 4 is parameter for interaction
myr<-round(cor(summary.data$LI,summary.data$param5),3)
...
plot(summary.data$param5,log(summary.data$p_interaction),col=as.factor(summary.data$lat))
r=cor.test(summary.data$LI,summary.data$param5)$estimate

plot(summary.data$LI,summary.data$param5,col=as.factor(summary.data$lat),main=r)
...
runexplore=0
if(runexplore==1){
w<-which(is.na(longdata$epoch))
longdata<-longdata[-w,]
numod <- gam(y ~ s(epoch)+s(relativetime)+side+POI+side*POI, k=5, bs="cr", data=longdata, method="REML", family="gaussian")
# #
# #
numod2 <- gam(y ~ s(relativetime, k=5, bs="tp")+
             s(side+POI,k=5,bs="re"), data=longdata, method="REML", family="gaussian")
# #
anova(numod,numod2,test="Chisq")
# #
draw(numod)
longdata$p<-predict(numod)
epochs=1:max(longdata$epoch)
g3<-ggplot(longdata[longdata$epochs%in%epochs,],aes(y=y,x=sec,colour=side))+
geom_line(aes(colour=side),alpha=0.4)+
geom_line(aes(y=p))+
theme_bw()+
theme(text=element_text(size=14))+
...
# #
# 
# d<-table(longdata$relativetime,longdata$epoch)
# d<-data.frame(unclass(d))
# colMeans(d)

source(here('plot_functions.R')) #From M Clark github  
...
numod <- gam(y ~relativetime, k=5, data=longdata) #THIS RUNS
numod <- gam(y ~ s(sec), k=5, data=longdata) #Doesnt run
numod <- gam(y ~ s(sec),  data=longdata) #RUNS
numod <- gam(y ~ sec, k=5, data=longdata) #RUNS
numod <- gam(y ~ s(sec)+s(sec,by=side), k=5, data=longdata) #Doesnt run
numod <- gam(y ~ sec+s(sec,by=side), k=5, data=longdata)#Doesnt run
numod <- gam(y ~ sec+s(sec,by=side),  data=longdata) #RUNS
numod <- gam(y ~ sec+s(POI,by=side),  data=longdata) #A term has fewer unique covariate combinations than specified maximum degrees of freedom

numod <- gam(y ~ relativetime +s(POI,by=side),  data=longdata) #A term has fewer unique covariate combinations than specified maximum degrees of freedom
numod <- gam(y ~ sec+ POI + side,  data=longdata) #THIS RUNS
numod <- gam(y ~ sec+ POI + side+ POI*side,  data=longdata) #THIS RUNS


#PLOT VS RELATIVE TIME
longdata %>% 
  ggplot(aes(relativetime, y)) +
  geom_point() + 
  geom_smooth(method = 'gam', formula = y ~ s(x)) + # the y and x are just generic names for the axes
  theme_trueMinimal()  #this works, with v poor fit
